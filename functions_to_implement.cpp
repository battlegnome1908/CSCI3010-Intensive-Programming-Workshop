...
// Student Answer: 
// I chose the EvenMask function for Lec4
// I chose the first two functions for Lec5:
//		Split
//		RemoveAllSubstrings
...
std::vector<bool> EvenMask(std::vector<int> numbers_vect){
    std::vector<bool> even_mask;
    	
	for (int num : numbers_vect) {
        even_mask.push_back(num % 2 == 0); // true if even, false if odd
    }
...
std::vector<std::string> Split(std::string whole, std::string separator) {
    ...
    size_t i = 0; // index
    while ((i = whole.find(separator)) != std::string::npos) {
        result.push_back(whole.substr(0, i)); // add to 
        whole.erase(0, i + separator.length());
    }
...
std::string RemoveAllSubstrings(std::string s1, std::string s2) {
    size_t i = 0; // index
    while ((i = s1.find(s2)) != std::string::npos) { // as long as a member of s2 exists in s1 or until reaches end of string (npos), this will continue
